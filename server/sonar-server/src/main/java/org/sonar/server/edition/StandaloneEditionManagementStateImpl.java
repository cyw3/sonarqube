...
public class StandaloneEditionManagementStateImpl implements MutableEditionManagementState, Startable {
  ...
  public void start() {
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      this.currentEditionKey = internalPropertyValues.getOrDefault(CURRENT_EDITION_KEY, Optional.empty()).orElse(null);
      this.pendingInstallationStatus = internalPropertyValues.getOrDefault(PENDING_INSTALLATION_STATUS, Optional.empty())
        .map(PendingStatus::valueOf).orElse(NONE);
      this.pendingEditionKey = internalPropertyValues.getOrDefault(PENDING_EDITION_KEY, Optional.empty()).orElse(null);
      this.pendingLicense = internalPropertyValues.getOrDefault(PENDING_LICENSE, Optional.empty()).orElse(null);
    }
  }
  ...
  @Override
  public synchronized PendingStatus startAutomaticInstallation(License license) {
  ...
  @Override
  public synchronized PendingStatus startManualInstallation(License license) {
    checkLicense(license);
  ...
  @Override
  public synchronized PendingStatus automaticInstallationReady() {
  ...
  public synchronized PendingStatus finalizeInstallation() {
    checkState(this.pendingInstallationStatus != AUTOMATIC_READY && this.pendingInstallationStatus != MANUAL_IN_PROGRESS,
      "Can't finalize installation when state is {}", this.pendingInstallationStatus);
