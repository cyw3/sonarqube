...
public class ApplyLicenseAction implements EditionsWsAction {
  ...
  public void handle(Request request, Response response) throws Exception {
    ...
    if (editionManagementState.getPendingInstallationStatus() != EditionManagementState.PendingStatus.NONE) {
      throw BadRequestException.create("Can't apply a license while applying one is already in progress");
    }
    ...
    if (license.contains("manual")) {
      editionManagementState.startManualInstallation(newLicense);
    } else {
      editionManagementState.startAutomaticInstallation(newLicense);
    }
  ...
  private WsEditions.StatusResponse buildResponse() {
    return WsEditions.StatusResponse.newBuilder()
      .setNextEditionKey(editionManagementState.getPendingEditionKey()
        .orElseThrow(() -> new IllegalStateException("Pending edition key should be set")))
